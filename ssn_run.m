function [E I Rmean] = ssn_run(params)
...
if Jii-Jei < 0 && Jii-Jei < Jie-Jee
    disp('Nonlinearity satisfied!')    
else
    disp('Not Nonlinear enough!')
end
%% Compute Weights
...
FBi = FBe;
% Feedback Parameters
sigmaFBe = deg2rad(90); % width it was 60 % 15
sigmaFBi = deg2rad(60); % width it was 60
muE = .1; % amplitude it was .3
...
switch makeitRND
    ...
    case 'no'
        tau = [normrnd(tauE,0.05 * tauE, [grid 1]); normrnd(tauI,0.05 * tauI, [grid 1])];
        n = ones(2*grid,1)*params.n; % n can range between 1.6<n<2.4
