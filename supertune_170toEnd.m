...
%                         firing(i,l,k,j,m) = vec(i) / (trialsPerThisFeature * ((endstim - tendMS) - startstim) / 1000);
    ...
    if sv == 1
        save(['E:\MT_MST\SuperTuneSpkTrains\ms_rmvd_170toEnd\',name,num2str(ch),num2str(unit),'spktrain_bl.mat'],'spktrain_bl','Fs')
        save(['E:\MT_MST\SuperTuneSpkTrains\ms_rmvd_170toEnd\',name,num2str(ch),num2str(unit),'spktrain.mat'],'spktrain','Fs','RFcenterIdx')
        save(['E:\MT_MST\SuperTuneFiringMatrix\ms_rmvd_170toEnd\',name,num2str(ch),num2str(unit),'firingMat.mat'],'firing')
        save(['E:\MT_MST\SuperTuneFiringMatrix\ms_rmvd_170toEnd\',name,num2str(ch),num2str(unit),'firingMat_bl.mat'],'firing_bl')
    end
...
for i = 1:size(MTnmsT,2)
    dur(i,1) = size(MTnmsT(i).sp(1).spktrain,1);
    for j = 1:size(MTnmsT(i).sp,2)
        rate(z,1) = sum(MTnmsT(i).sp(j).spktrain(:))/(4*dur(i,1)/10000); % NMS Translation
        rate(z,2) = sum(MTmsT(i).sp(j).spktrain(:))/(4*dur(i,1)/10000); % MS Translation
        rate(z,3) = sum(MTnmsS(i).sp(j).spktrain(:))/(4*dur(i,1)/10000); % NMS Spirals
        rate(z,4) = sum(MTmsS(i).sp(j).spktrain(:))/(4*dur(i,1)/10000); % MS Spirals
        z = z + 1;
    end
end
